// Most of code in this file is come from
...
function WebpackManifestPlugin(opts) {
  this.opts = _.assign({
    publicPath: null,
    basePath: '',
...
WebpackManifestPlugin.getCompilerHooks = (compiler) => {
  let hooks = compilerHookMap.get(compiler);
  if (hooks === undefined) {
    const SyncWaterfallHook = require('tapable').SyncWaterfallHook;
    hooks = {
      afterEmit: new SyncWaterfallHook(['manifest', 'outputPath'])
    };
...
WebpackManifestPlugin.prototype.getFileType = function(str) {
  str = str.replace(/\?.*/, '');
...
WebpackManifestPlugin.prototype.apply = function(compiler) {
  const moduleAssets = {};
  ...
  const emit = function(compilation, compileCallback) {
    ...
    const outputName = `manifest.${outputHash}.json`;
    if (isLastEmit) {
      ...
      if (this.opts.writeToFileEmit) {
        const outputFilePath = path.resolve(outputFolder, outputName);
        fse.outputFileSync(outputFilePath, output);
      }
    }

    if (compiler.hooks) {
      WebpackManifestPlugin.getCompilerHooks(compiler).afterEmit.call(manifest, outputName);
    } else {
  ...
  if (compiler.hooks) {
    ...
    if (!Object.isFrozen(compiler.hooks)) {
      compiler.hooks.webpackManifestPluginAfterEmit = WebpackManifestPlugin.getCompilerHooks(compiler).afterEmit;
    }
